...
export const LoginView: React.FC<LoginViewProps> = ({ 
  ...
  const selectedPreset = LOGIN_PRESET_BACKGROUNDS.find(p => p.id === loginSettings.bgPresetId) || LOGIN_PRESET_BACKGROUNDS[0];
  const selectedGradient = LOGIN_GRADIENT_OPTIONS.find(g => g.id === loginSettings.bgGradient) || LOGIN_GRADIENT_OPTIONS[0];

  const hasImageBg = loginSettings.bgType === 'preset' || (loginSettings.bgType === 'custom' && loginSettings.bgCustomUrl);
  const bgImageUrl = loginSettings.bgType === 'preset'
    ? selectedPreset.imageUrl
    : loginSettings.bgCustomUrl || selectedPreset.imageUrl;

  return (
    <div className="relative min-h-screen flex flex-col justify-between py-6 px-4 sm:px-6 lg:px-8 overflow-hidden bg-slate-900">
        ...
        <>
          <div 
            ...
            style={{
              ...
              filter: `blur(${loginSettings.bgBlurAmount || 0}px)`,
          ...
          <div 
            ...
            style={{ opacity: (loginSettings.bgOverlayOpacity ?? 45) / 100 }}
          />
        </>
      ) : (
        /* Gradient Background */
        <div className={`absolute inset-0 bg-gradient-to-br ${selectedGradient.cssClass} pointer-events-none`} />
      )}
      ...
      <header className="relative z-10 max-w-6xl mx-auto w-full flex items-center justify-between py-2">
        <div className="flex items-center gap-3 bg-white/95 backdrop-blur-md px-3.5 py-2 rounded-2xl border border-white/40 shadow-md">
          ...
          <div className="border-l border-slate-300 pl-3">
            ...
            <p className="text-[11px] text-slate-500 font-medium">Direktorat Pendidikan Non Gelar (DPNG)</p>
          </div>
        </div>
      ...
      {/* Main Container */}
      <main className="relative z-10 max-w-4xl mx-auto w-full my-auto py-6">
        <div className="bg-white rounded-2xl shadow-2xl border border-white/30 overflow-hidden grid grid-cols-1 lg:grid-cols-12 backdrop-blur-xs">
          
          {/* Left Hero / Info Branding Panel */}
          <div className="lg:col-span-5 bg-gradient-to-br from-[#002B66] via-[#002252] to-[#001736] p-7 sm:p-8 text-white flex flex-col justify-between relative overflow-hidden">
            {/* Background Decorative Circles */}
            <div className="absolute -top-12 -right-12 w-48 h-48 rounded-full bg-[#FDB913]/15 blur-2xl pointer-events-none" />
            ...
            <div className="relative z-10">
              ...
              <p className="text-xs sm:text-sm text-slate-200/90 leading-relaxed font-normal mb-8">
                {loginSettings.subjudulLogin || 'Sistem Informasi Manajemen Data Peserta Pendidikan Non Gelar Universitas Padjadjaran.'}
              </p>

              {loginSettings.showFeatureHighlights && (
                  ...
                  <div className="flex items-start gap-3 bg-white/5 p-3 rounded-xl border border-white/10 backdrop-blur-xs">
                    ...
                    <div>
                      ...
                      <p className="text-slate-300 text-[11px] leading-relaxed">
                        Luhung, Executive Education, Kredensial Mikro, Summer School, hingga Eduventure.
                      </p>
                    </div>
                  </div>
            ...
            <div className="relative z-10 pt-6 mt-6 border-t border-white/10 text-[11px] text-slate-400 flex items-center justify-between">
              ...
              <span className="text-amber-300/90 font-mono font-bold">v3.2 Production</span>
            </div>
          </div>
          ...
          <div className="lg:col-span-7 p-6 sm:p-8 lg:p-10 flex flex-col justify-center">
            ...
            {/* Announcement Banner if configured by Super Admin */}
            {loginSettings.showAnnouncement && loginSettings.announcementText && (
              <div className={`mb-4 p-3.5 rounded-xl border text-xs flex items-start gap-2.5 ${
                loginSettings.announcementType === 'warning'
                  ? 'bg-amber-50 border-amber-200 text-amber-900'
                  : loginSettings.announcementType === 'success'
                ...
                {loginSettings.announcementType === 'warning' ? (
                  ...
                  <Megaphone className="w-4 h-4 text-[#002B66] shrink-0 mt-0.5" />
                )}
                <div className="flex-1 font-medium leading-relaxed">
                  {loginSettings.announcementText}
                </div>
              </div>
            )}
            ...
            {/* Optional SSO Google Unpad Section */}
            {loginSettings.allowGoogleSso && (
              <>
      ...
      {/* Footer */}
      <footer className="relative z-10 max-w-6xl mx-auto w-full text-center text-xs text-white/80 py-2">
        <div className="inline-block bg-black/40 backdrop-blur-md px-4 py-2 rounded-xl border border-white/10 text-center">
          <p className="font-medium">© 2026 Direktorat Pendidikan Non Gelar — Universitas Padjadjaran. Hak Cipta Dilindungi.</p>
